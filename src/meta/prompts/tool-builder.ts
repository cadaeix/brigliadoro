...
2. A **\`createX(...stores?, rng?)\` factory** that wraps the pure function in an MCP \`tool()\` handler.
...
**Thread rng through the factory too**, not just the pure / step function. The factory accepts \`rng: () => number = Math.random\` as its last parameter (after any stores), and threads that \`rng\` into every pure-function or step-function call inside the handler. \`server.ts\` builds tools without passing rng, so they default to Math.random at runtime — no behavioural change. But the validator's handler-integration tests (the layer that catches wrong session-write signatures, wrong \`(entity, key)\` pairs, missing persistence calls — bugs that pure-function tests can't see) need to seed the handler's RNG to force specific branches deterministically. Without an injectable rng on the factory, those tests can't fire reliably and a real bug class slips through. See \`#rng-threading-and-handler-determinism\` for the worked pattern.
...
2. **RNG threading.** For any tool that touches an RNG primitive: pure function (or step function) accepts \`rng: () => number = Math.random\`. The factory accepts the same parameter — last, after stores — and threads it into every pure-function / step-function call inside the handler. \`server.ts\` does not pass rng (defaults to Math.random); the parameter exists for the validator's handler-integration tests. Pure-function-takes-rng-but-handler-doesn't is the failure shape that produces flaky handler tests and silently-untested session writes. \`#rng-threading-and-handler-determinism\`.
